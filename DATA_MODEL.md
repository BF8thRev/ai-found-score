...
| 9 | Offer | `sources`, `listings`, `issues` | Only tiers in `OFFERED_TIERS` (`public/js/config.js`; Sep 2026: `snapshot` $29 and `before_after` $59); no button or link is ever rendered for another tier. The $29 Fix steps is offered only with **≥ 3 fix items** (`issues`, `MIN_FIX_ITEMS` / `snapshotOffered()` in `shared/report-v2.js`; the refund promise). Normal: $59 primary, $29 link. Named in every answer, or no mismatches and no missing sources: the $29 Fix steps (baseline fixes still apply) plus a quiet "Monthly monitoring is coming." (no price). |
...
- Locking (unpaid, `src/lib/lock.js` `lockReport`): every listing that is not `match` keeps only `platform` + `status`; issues keep only `kind` + `severity` + `title` (`description`, `steps` and `copyText` are withheld). Everything else is the free report, `ownerDescriptors` included. Paid: `locked: false`.
