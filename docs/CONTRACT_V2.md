...
- `MIN_FIX_ITEMS` (3) / `fixItems(report)` / `xrayOffered(report)` (and the older alias `snapshotOffered`): the $49 AI Visibility X-Ray is offered only when the report has ≥ 3 fix items (`issues`). `report.js` mirrors the constant.
- `buildGapSheet(report)` / `buildFixChecklist(report)` / `xraySections(report)`: the X-Ray sections, from report data only (competitors proven by 2+ answers; cited sources with `youListed: false` that list them). Served only unlocked (`src/lib/lock.js`).
